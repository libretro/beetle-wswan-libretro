...
static INLINE void WriteMem(uint32 A, uint8 V, bool WW)
{
   uint32 offset, bank;
...
static INLINE uint8 ReadMem(uint32 A, bool WW)
{
   uint32 offset, bank;
...
static INLINE uint8 ReadPort(uint32 number, bool WW)
{
   number &= 0xFF;
...
static INLINE void WritePort(uint32 IOPort, uint8 V, bool WW)
{
   IOPort &= 0xFF;
...
void WSwan_MemoryLoadNV(void)
{
   if(IsWW)
   {
      ...
      RFILE *flash_fp = filestream_open(path.c_str(),
      ...
      filestream_read(flash_fp, wsCartROM, 524288);
...
void WSwan_MemorySaveNV(void)
{
   if(IsWW)
   {
      ...
      filestream_write(flash_fp, wsCartROM, 524288);
      filestream_close(flash_fp);
   }
}
...
int WSwan_MemoryStateAction(StateMem *sm, const unsigned load, const bool data_only)
{
   SFORMAT StateRegs[] =
   {
      SFVARN(wsRAM, "RAM"),
   ...
   if(load)
   {
      ...
      if(IsWW && load < 0x94100)
      {
         ...
         MDFNSS_StateAction(sm, load, data_only, CompatStateRegs, "MEMR", false);
